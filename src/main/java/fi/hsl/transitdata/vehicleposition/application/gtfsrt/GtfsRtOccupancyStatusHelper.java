...
public class GtfsRtOccupancyStatusHelper {
     ...
     */
    public GtfsRtOccupancyStatusHelper(
            NavigableMap<Integer, GtfsRealtime.VehiclePosition.OccupancyStatus> occuToOccupancyStatus,
        ...
        this.passengerCountEnabledVehicles = (passengerCountEnabledVehicles == null
                || passengerCountEnabledVehicles.isEmpty()) ? null : new HashSet<>(passengerCountEnabledVehicles);
    ...
    public Optional<GtfsRealtime.VehiclePosition.OccupancyStatus> getOccupancyStatus(Hfp.Payload hfpPayload,
            PassengerCount.Payload passengerCountPayload) {
        if (passengerCountEnabledVehicles == null
                || passengerCountEnabledVehicles.contains(hfpPayload.getOper() + "/" + hfpPayload.getVeh())) {
            ...
            if (passengerCountPayload != null) {
                if (passengerCountPayload.getVehicleCounts().hasVehicleLoad()
                        && passengerCountPayload.getVehicleCounts().getVehicleLoad() <= 0) {
                ...
                return Optional.of(loadRatioToOccupancyStatus
                        .lowerEntry(passengerCountPayload.getVehicleCounts().getVehicleLoadRatio()).getValue());
            }
